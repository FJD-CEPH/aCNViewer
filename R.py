...
class R:
	...
	def getLibraryPathList(self):
		...
		return [dirName.strip().split()[-1].strip('"') for dirName in self._execString('.libPaths()', True)]
	...
	def installPackageFromUrl(self, url):
		...
		fileName = WebExtractor()._getStrIncludedInTag(content, '<td> Package&nbsp;source: </td>\n<td> <a href="', '"')
	...
	def _getStrFromList(self, valueList):
		...
		if valueList and False not in isFloatSet:
			...
			valueList = [self.__getValueStrForList(value) for value in valueList]

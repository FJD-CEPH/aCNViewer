...
class ProcessDataWithProgressNotifyer:
	def __init__(self, step, getDataSize = True, cluster = None, callBackFunc = None, *callBackArgs):
		self.step = step
	...
	def process(self, dataList, functionToDo, *args):
		...
		for data in dataList:
			...
			if int(nb) % self.step == 0:
				Log.info(str(nb) + ', ' + str(nb / totalNb) + ', ' + Utilities.getTimeString())
		if self.cluster:
...
def importModule(moduleName, moduleToImport = '*'):
	...
	if moduleToImport is None:
		...
		return
	__import('from %s import %s' % (moduleName, moduleToImport), 'from %s import %s' % (shortModuleName, moduleToImport))
...
def runFuncAndSendmail(emailList, func, *args):
	...
	startTime = time.time()
	if type(emailList) == types.StringType:
		emailList = [emailList]
	elif type(emailList) != types.ListType:
		args = (func, ) + args
		func = emailList
		emailList = [email]
	...
	print msg
	SendMail().send('vrenault@cephb.fr', emailList, subject, 'Program took %d s on "%s" by user "%s", function %s, parameters = %s\n%s' % (time.time() - startTime, os.environ.get('HOSTNAME', '?'), os.environ.get('USER', '?'), func, args, msg))
...
class Utilities:
	...
	def getFileLastEditedDate(fileName):
		...
		return time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(fileStat[stat.ST_MTIME]))
	...
	#def getFunctionResultWithCache2(functionName, *args):
		#fileName = '_' . join([str(functionName)] + [str(arg) for arg in args])
		#print fileName
	@staticmethod
	...
	def mySystem(cmd, scriptName = None, allowFailure = False):
		...
		if allowFailure:
			return
		if code:
			raise NotImplementedError('cmd "%s" failed with code %d on host %s' % (cmd, code, os.environ.get('HOSTNAME', '?')))
	...
	def system(cmd, raiseError = True, shell = False):
		try:
			...
			outFh.close()
			cmdList = cmd.split(' ')
			if 'cd' in cmdList or '<' in cmd or '>' in cmd or '|' in cmd or 'rm' in cmdList:
				shell = True
			if shell:
				cmdList = cmd
			if subprocess.__dict__.has_key('check_call'):
	...
	def processLoopWithProgressNotifyer(dataList, functionToDo, *args):
		ProcessDataWithProgressNotifyer(1000).process(dataList, functionToDo, *args)
	...
	def getLastUpdatedDateForFile(fileName):
		fileStat = os.stat(fileName)
		return time.strftime("%a %b %d %H:%M:%S %Y", time.localtime(fileStat[stat.ST_MTIME]))
	
	@staticmethod
	def getLastUpdatedDateForFile2(fileName):
		fileStat = os.stat(fileName)
		return "%04d-%02d-%02d %02d:%02d:%02d" % time.localtime(fileStat[stat.ST_MTIME])[:6]

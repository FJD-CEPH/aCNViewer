...
class ValueParser:
    ...
    def getIdxListFromIntervalStrOrList(self, intervalStrOrList,
                                        allowFloat=False):
        idxList = []
    ...
    @staticmethod
    def isNb(nb):
        if isinstance(nb, types.IntType):
            return True
    ...
    def isFloat(nb):
        ...
        if type(nb) in (types.StringType, types.UnicodeType):
            ...
            if len(partList) not in [1, 2]:
                return
            return set([ValueParser.isNb(part) for part in partList]) == \
                set([True])

    @staticmethod
    def __isNone(value):
        return value is None or (type(value) in
                                 (types.StringType, types.UnicodeType) and
                                 (value == str(None) or value == ''))
    ...
    def assertValueIsStringAndGetValue(value):
        if type(value) not in (types.StringType, types.NoneType,
                               types.UnicodeType):
    ...
    def checkValue(value, commandKeyClass, allowNullValue=True):
        ...
        if value not in allowedValueList:
            Log.error('value = "' + str(value) + '" not in ' +
                      str(commandKeyClass.getKeys()))
            raise NotImplementedError
        return value

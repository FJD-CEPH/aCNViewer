"""
ObjectBase.py
    - 'ObjectBase': Base class for all basic structure class like Position,
    Sequence, etc.
    - 'CommandKey': the basic class which store all the information about one
    HTML variable: name and all possible values
...
def myStr(self):
    ...
    if not hasattr(self, '__dict__'):
        if isinstance(self, types.ListType) or isinstance(self,
                                                          types.TupleType):
            stringValue = ''
            for elt in self:
    ...
    for attrName in attrNameList:
        ...
        isList = isinstance(currentAttr, types.ListType)
        isDict = isinstance(currentAttr, types.DictType)
...
def init(obj, *args):
    ...
    localDict = args[0]
    for arg in argList:
        # case where we have one class 'A' which inherits ObjectBase and
        # another one 'B' which inherits 'A' and which has more parameters
        # than 'A' and which constructor calls the one from 'A': in that case,
        # 'argList' will contain some variables that 'localDict' won't have
        if arg not in localDict:
            continue
...
class SimpleObjectBase(object):
    ...
    def getObjFromParamDict(self, attrDict, obj):
        ...
        argList = [obj.getattr(attrDict, eltNameDict[argName])
                   for argName in argList[1:]]
        obj = self(*argList)
        # print obj
        return obj
    ...
    def __checkObjectIsSameClass(self, other):
        if not self.getClassName() == other.getClassName():
            if config1.getboolean('TEST'):
                print 'can not compare %s with %s: ' % (self.__class__,
                                                        other.__class__)
            raise MemoryError
    ...
    def __isDict(self, obj):
        return isinstance(obj, types.DictType) or isinstance(obj, MultiDict)
    ...
    def __eq__(self, other, printDiffField=False):
        ...
        if config1 and config1.TRUE_COMPARE == 'True':
    ...
    def __str__(self, printType=False):
        ...
        attrNameList = self.__dict__.keys()
        attrNameList.sort()
        # we remove 'self' from the field list to avoid maximum recursion
        # exception
        if 'self' in attrNameList:
            attrNameList.remove('self')
        for attrName in attrNameList:
            ...
            currentVal = None
            if not isList and not isDict:
                if isinstance(currentAttr, types.StringType):

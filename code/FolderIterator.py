...
class FolderIterator:
    ...
    def _updateFileListFromDirNameAndFileExt(self, dirName, fileList,
                                             isFileToProcessFunc, *args):
        # print 'd', dirName
        for fileName in glob.glob(os.path.join(dirName, '*')):
            ...
            if not self._isDir and os.path.isdir(fileName):
                if not self.followLinks and os.path.islink(fileName):
                    ...
                    continue
                self._updateFileListFromDirNameAndFileExt(
                    fileName, fileList, isFileToProcessFunc, *args)
    ...
    def __doesFileHasExtension(self, fileName, fileExt):
        return (not self._isDir and os.path.isfile(fileName) and
                ((isinstance(fileExt, types.ListType) and
                  self.__doesFileHasExtensionInList(fileName, fileExt)) or
                 ('.' not in fileExt and
                  Utilities.getFileExtension(fileName) == fileExt or
                  fileExt == '*') or
                 (os.path.basename(fileName)[-len(fileExt):] == fileExt))) or \
               (self._isDir and os.path.basename(fileName)[-len(fileExt):] ==
                fileExt)

    def getFileListInDirWithExt(self, dirName, fileExt, cacheFileName=None):
        if cacheFileName:
            return Utilities.getFunctionResultWithCache(
                cacheFileName, self._getFileList, dirName,
                self.__doesFileHasExtension, fileExt)
    ...
    def _getFileList(self, dirName, isFileToProcessFunc, *args):
        ...
        if not self._isDir and os.path.isfile(dirName):
            ...
            if isFileToProcessFunc(dirName, *args):
                ...
                self._updateFileListFromDirNameAndFileExt(
                    dirName, fileList, isFileToProcessFunc, *args)
        else:
    ...
    def __isSeqFile(self, fileName, allowFailed):
        ...
        seqEnd3 = '.fastq.gz'
        isFileToProcess = os.path.basename(fileName)[-len(seqEnd):] == \
            seqEnd or \
            os.path.basename(fileName)[-len(seqEnd2):] == \
            seqEnd2 or \
    ...
    def __getBaseNameForFile(self, fileName):
        ...
        if not readIdx:
            raise NotImplementedError(
                'Could not find readIdx for file %s' % fileName)
    ...
    def __getSortedSeqFileList(self, seqFileList):
        ...
        for seqFile in seqFileList:
            ...
            ).getSampleNameRunNameAndUplexFromPath(seqFile)
            laneNb, readNb = IlluminaRun().getLaneAndReadNbfromFile(seqFile)
            newSeqFileList.append(
                (sampleName, fcName, laneNb, readNb, seqFile))
        newSeqFileList.sort()
        return [seqInfo[-1] for seqInfo in newSeqFileList]

    def _iterSeqFiles(self, seqFileList, allowReadNbToBeTheSame=False,
                      sort=True):
        if sort:
            seqFileList.sort()
        ...
        prevSampleInfo = IlluminaRun().getSampleNameRunNameAndUplexFromPath(
            prevSeqFile)
        lastReturnedSeqFile = None
        if not self.__doesSeqFileNameContainReadNb(prevSeqFile) and \
           allowReadNbToBeTheSame:
        ...
        for seqFile in seqFileList:
            laneNb, readNb = IlluminaRun().getLaneAndReadNbfromFile(seqFile)
            isSingleEnd = False
            currentSampleInfo = IlluminaRun().\
                getSampleNameRunNameAndUplexFromPath(seqFile)
            # print '))', currentSampleInfo, laneNb, readNb
            if currentSampleInfo[0] == prevSampleInfo[0] and \
               laneNb == prevLaneAndReadNb[0]:
                if not allowReadNbToBeTheSame and \
                   readNb == prevLaneAndReadNb[1]:
                    raise NotImplementedError(
                        'Read nb should be different for seqFile %s' % seqFile)
                # print laneNb, allowReadNbToBeTheSame, prevSeqFile,
                # seqFile,lastReturnedSeqFile
                if allowReadNbToBeTheSame and \
                   os.path.dirname(prevSeqFile) != os.path.dirname(seqFile):
                    if lastReturnedSeqFile != prevSeqFile:
                        lastReturnedSeqFile = prevSeqFile
                        yield laneNb, prevSeqFile
                else:
                    if isOldFormattedSeqName or \
                       self.__getBaseNameForFile(prevSeqFile) == \
                       self.__getBaseNameForFile(seqFile):
                        lastReturnedSeqFile = seqFile
                        prevSampleInfo = \
                            IlluminaRun().\
                            getSampleNameRunNameAndUplexFromPath(
                                prevSeqFile)
                        sampleInfo = \
                            IlluminaRun().\
                            getSampleNameRunNameAndUplexFromPath(
                        ...
                        if prevSampleInfo != sampleInfo:
                            raise NotImplementedError('Sample information do \
not match for files %s and %s: %s != %s' % (prevSeqFile, seqFile,
                                            prevSampleInfo, sampleInfo))
        ...
        while len(seqFileList):
            ...
            if os.path.dirname(seqFile) != os.path.dirname(seqFile2):
                if 'single.fq.gz' in seqFile:
                    ...
                    raise NotImplementedError(
                        '%s and %s should be paired' % (seqFile, seqFile2))
            else:
                singleSeqFile = None
                if seqFileList:
    ...
    def getDataIterator(self, seqDir, fileExt, isSeq=False):
        ...
        if fileExt:
            ...
            if fh.read().strip():
                seqFileList = list(
                    self.getFileListInDirWithExt(seqDir, '.fq.gz'))
                seqFileList.sort()
                iterator = self.__trimmedSeqIterator(seqFileList)
            else:
                seqFileList = self.getSeqFileListInDir(seqDir)
                seqFileList = [(IlluminaRun().
                                getSampleNameRunNameAndUplexFromPath(seqFile),
                                IlluminaRun().getLaneAndReadNbfromFile(
                                    seqFile), seqFile) for seqFile in
                               seqFileList]
                seqFileList.sort()
                seqFileList = [seqFile for info1,
                               info2, seqFile in seqFileList]
                if seqFileList:
                    ...
                    if ValueParser().isNb(dirName):
                        seqFileList = [(IlluminaRun().
                                        getSampleNameRunNameAndUplexFromPath(
                                            seqFile), IlluminaRun().
                                        getSampleNbFromFile(seqFile),
                                        IlluminaRun().getLaneAndReadNbfromFile(
                                            seqFile), seqFile) for seqFile in
                                       seqFileList]
                        seqFileList.sort()
                        seqFileList = [seqFile for info1, info2,
                                       info3, seqFile in seqFileList]
    ...
    def __getFileListFromRemoteServer(self, srcDir, ssh, withinMinutes):
        ...
        if '*' in baseName:
            ...
            if '{' in baseName:
                ...
                baseName = WebExtractor()._getStrIncludedInTag(
                    baseName, '{', '}')
        ...
        if fileExtList in [[''], []] and '*' in srcDir:
            ...
            fileList = ['%s:%s' % (hostname, fileName)
                        for fileName in stdout.read().strip().split('\n')]
            print 'FILES', fileList
        else:
            print 'Getting file list from %s in %s with ext %s' % \
                  (hostname, srcDir, fileExtList)
            cmd = 'find %s -type f -follow' % (srcDir + os.path.sep)
            if fileExtList:
                cmd += ' %s' % ' -o '.join(['-name "*.%s"' %
                                            fileExt for fileExt in
                                            fileExtList])
            if withinMinutes:
            ...
            fileList = ['%s:%s' % (hostname, fileName) for fileName in
                        fileList if not fileExtList or
                        (fileExtList and self.__doesFileHasExtension(
                            fileName, fileExtList))]
    ...
    def getFileListInDir(self, srcDir, ssh=None, withinMinutes=None):
        hostname = None
        if ':' in srcDir:
            fileList = self.__getFileListFromRemoteServer(
                srcDir, ssh, withinMinutes)
        else:
...
def run(options, args):
    for f in FolderIterator().getDataIterator(options.dirName,
                                              options.pattern):
        print f
        if len(f) != 3:
            raise NotImplementedError

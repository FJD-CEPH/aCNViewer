#---Standard modules---#000000#FFFFFF-------------------------------------------
import string
import types
import os

#---DbSnp modules---#000000#FFFFFF----------------------------------------------
...
class Position(ObjectBase):
	def __init__(self, ctgId, start, end, allowNegativeValues = False):
		...
		if self.end < self.start:
			raise NotImplementedError('Position is inconsistent: start %s is < end %s' % (start, end))
		if not allowNegativeValues and (start and start < 0) or (end and end < 0):
			raise NotImplementedError('Position can not have negative values: start = %s, end = %s' % (start, end))
	...
	def getMergedPosList(posList):
		if type(posList[0] == types.TupleType):
			posList.sort()
		else:
		...
		for pos in posList:
			...
			if pos.isMergeable(currentPos):
				pos = Position(pos.ctgId, min(pos.start, currentPos.start), max(pos.end, currentPos.end))
			else:
	...
	def setCtgId(self, ctgName, locusCtgId, locusId, objectName):
		if self.ctgId != locusCtgId:
			print "Inconsistency in db for locus [%d]: %s's ctgId [%s] and locus ctgId [%d] are different!!!" % \
	...
	def getPositionDifference(self, other):
		''' create a list from the (start, end) of each position, sort the list and return the difference between the 3rd and the 2nd element '''
	...
	def getOverlapPosition(self, pos):
		...
		if (first.start <= second.start and first.end >= second.start):
			result = Position(self.ctgId, second.start, min(first.end, second.end))
			return result
		return None
	...
	def isIncludedIn(self, otherPos):
		if otherPos.ctgId != self.ctgId or self.start < otherPos.start or self.end > otherPos.end:
			return False
		return True

...
class Journal(SimpleObjectBase):
    ...
    def __init__(self, name, date, volume, pages, impactFactor=None,
                 issnNb=None, aliasList=None):
        init(self, locals())
    ...
    def getJournalDateVolumeAndPagesFromStr(journalInfoStr):
        ...
        if matchObj is None:
            if 'ahead of' in journalInfoStr:
                return journalInfoStr.split(' [')[0],\
                       Utilities.getShortTimeString()[:4], '', ''
            print 'Journal info [%s] not handled' % journalInfoStr
            raise NotImplementedError
        journalName = matchObj.group(1)
        if '.' in journalName:
            print 'Journal [%s] is weird... [%s]' % (journalName,
                                                     journalInfoStr)
            raise NotImplementedError
...
class Article(SimpleObjectBase):
    ...
    def __init__(self, idList, title, abstract, authors, institute, journal,
                 keywords=None, linkList=None, publishedDate=None, key=None):
        if not isinstance(idList, types.ListType):
            idList = [idList]
        init(self, locals())

    def getImpactFactor(self):
        return ['%0.2f' % impactFactor for impactFactor in
                self.journal.impactFactor]
...
class WebExtractor(object):
    ...
    def _getStrIncludedInTag(self, text, tag, endTag=None):
        ...
        # print 1, [tag, startIdx, endTag, self._endIdx,
        # text[startIdx+len(tag): startIdx+100]]
        if startIdx == -1 or self._endIdx == -1:
            return
        self._endIdx += startIdx + len(tag)
        # print 2, [tag, startIdx, endTag, self._endIdx,
        # text[startIdx+len(tag): startIdx+100]]
    ...
    def _getPageContentForUrl(self, url):
        ...
        for i in xrange(self._nbRetries):
            ...
            if self._endPage == '' or endKeyword == self._endPage:
                return content
            print 'Retrying [%s]: endPage should be "%s" but "%s" found' % \
                  (url, self._endPage, [endKeyword])
...
class EasilyArticleReadableInViWriter(ArticleWriterBase):
    ...
    def _write(self, fileName, articleList):
        articleList.sort(lambda article1, article2: cmp(
            (article1.publishedDate, article1.title), (article2.publishedDate,
                                                       article2.title)))
        fh = open(fileName, 'w')
        ...
        for article in articleList:
            fh.write('keyword: %s\npubmedId: %d\npublishedDate: %s\nauthors: \
%s\nTITLE: %s\njournal: %s\nimpact factor: %s\n\n%s\n\n\n' %
                     (article.key, int(article.getPubmedId()),
                      article.publishedDate, article.authors,
                      article.title, article.getJournalInfo(),
                      article.getImpactFactor(), article.abstract))
        fh.close()
...
class FullArticleWriter(ArticleWriterBase):
    ...
    def _write(self, fileName, articleList):
        articleList.sort(lambda article1, article2: cmp(
            article2.journal.impactFactor, article1.journal.impactFactor))
        fh = CsvFileWriter(fileName)
        fh.write(['Keyword', 'Title', 'Autors', 'Journal',
                  'Impact factor', 'Institute', 'Links', 'Abstract'])
        for article in articleList:
            fh.write([article.key, article.title, article.authors,
                      article.getJournalInfo(), article.getImpactFactor(),
                      article.institute, ','.join(article.linkList),
                      article.abstract])
...
class ArticleExtractorBase(object):
    ...
    def _getStrIncludedInTag(self, content, startTag, endTag=None):
        resStr = self._webExtractor._getStrIncludedInTag(
    ...
    def getArticleListByKeyword(self, keyword):
        ...
        print '%d article(s) extracted for keyword [%s]' % (len(urlList),
                                                            keyword)
        articleList = []
        for url in urlList:
    ...
    def process(self, keywordList, fileName='abstracts', dirName=None,
                writerShortName=FULL_WRITER):
        ...
        self.writeDict(self._getFileNameFromDirAndFileName(
            dirName, self._fileName + '_%s.csv' % self.__dateStr),
                       articleDict, writerClass)
        if self._saveProcessedIdList:
            Utilities.saveCache(self._processedIdList,
                                self._getFileNameFromDirAndFileName(
                                    dirName, fileName + '_%s.pyDump' %
                                    self.__dateStr))
...
articleWriterDict = {
    'vi': EasilyArticleReadableInViWriter, 'full': FullArticleWriter}

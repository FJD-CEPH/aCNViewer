...
''' This class is designed to contain the different values associated to one
variable. Here the variable name is 'name' and to define its different
possible values, it is sufficient to define them at the same level than 'name'.
It is assumed that all the variable which name are upper case are one possible
...
class CommandKey:
    ...
    # by default, we return all the variables of the class which are in upper
    # case
    def getKeys(self):
        keys = []
...
class CommandParameter:
    _typeDict = {CommandParameterType.BOOLEAN: CommandParameterType.INT,
                 CommandParameterType.DB: CommandParameterType.STRING,
                 CommandParameterType.COMMA_SEP: CommandParameterType.STRING,
                 CommandParameterType.COMMA_SEP_ID:
                 CommandParameterType.STRING,
                 CommandParameterType.DB_LIST: CommandParameterType.STRING,
                 CommandParameterType.COMMA_SEP_FLOAT:
                 CommandParameterType.STRING}

    def __init__(self, optionName, definition, commandType=None, varName=None,
                 defaultValue=None, helpString='',
                 action='store'):
        if not commandType:
        ...
        if varName is None:
            varName = definition
        if not isinstance(varName, types.StringType):
    ...
    def addOption(self, optionParser):
        ...
        optionParser.add_option(*optList, action=self.action,
                                type=self.commandType, dest=self.varName,
...
def runFromTerminal(name, commandParameterList, mainFunctionToRun,
    ...
    for commandParameter in commandParameterList:
        ...
        if commandParameter.originalType == CommandParameterType.BOOLEAN:
            ...
            value = valueList
        elif commandParameter.originalType in \
            (CommandParameterType.COMMA_SEP_ID,
            ...
            if value is None:
                ...
                if not isinstance(value, types.ListType):
                    value = value.split(',')
                if commandParameter.originalType == \
                   CommandParameterType.COMMA_SEP_ID:
                    value = ValueParser().getIdxListFromIntervalStrOrList(
                        value)
                elif commandParameter.originalType == \
                        CommandParameterType.COMMA_SEP_FLOAT:
                    value = ValueParser().getIdxListFromIntervalStrOrList(
                        value, True)
